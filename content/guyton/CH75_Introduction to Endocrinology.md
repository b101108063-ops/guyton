...
book: "Guyton & Hall"
summary: "Chapter 75 from Guyton & Hall Textbook of Medical Physiology, 14th Edition. Content extracted from official PDF source."
key_points:
...
**Table 75-1** 主要內分泌腺體、激素及其功能：

| 腺體 | 激素 | 主要功能 | 化學結構 |
|---|---|---|---|
| **下丘腦** | TRH、CRH、GHRH、GnRH、Dopamine | 調控垂體前葉激素分泌 | Peptide |
| **垂體前葉** | GH、TSH、ACTH、Prolactin、FSH、LH | 調控甲狀腺、腎上腺皮質、乳房、生殖腺 | Peptide |
| **垂體後葉** | ADH（vasopressin）、Oxytocin | 腎水份重吸收、子宮收縮、泌乳 | Peptide |
| **甲狀腺** | Thyroxine（T4）、T3、Calcitonin | 升高基礎代謝率、調節血鈣 | Amine/Peptide |
...
| **心臟** | ANP | 鈉排泄、降血壓 | Peptide |
| **脂肪組織** | Leptin | 抑制食慾、刺激產熱 | Peptide |
...
激素在血液中的濃度極低（1 pg/ml 至數 μg/ml），需要高度專一的回饋控制機制才能精確調控。
...
激素分泌具有晝夜節律（diurnal rhythm），例如生長激素在睡眠早期分泌增加。這主要由下丘腦的 suprachiasmatic nucleus（SCN）——也就是身體的「主時鐘」——所調控。
...
Steroid 與甲狀腺激素在血漿中主要與蛋白質結合。例如 99% 以上的 thyroxine 與血漿蛋白結合。結合型激素不能輕易穿越微血管，是生物學上無活性的儲備型態；游離型激素才能與目標細胞受器結合並發揮作用。與蛋白質結合也大幅延長激素的半衰期。

### 激素的清除（Metabolic Clearance Rate）
...
激素作用的第一步是與目標細胞上的特定受器結合。缺乏受器的細胞對該激素不發生反應。每個目標細胞通常含有 2000–100,000 個特定受器。
...
受器數量並非固定不變。受器可被下調（down-regulation）——激素濃度升高時，目標細胞會減少受器數量或使其內化（internalization），導致對該激素的敏感性降低。上調（up-regulation）則使組織對激素越來越敏感。
...
這是最大一類膜受器（約 1000 種），具有七次跨膜結構域。受器被激素活化後，經由異源三聚體 G 蛋白（Gs 或 Gi）傳遞訊號。活化後的 Gs 蛋白刺激 adenylyl cyclase 產生 cAMP；Gi 蛋白則抑制 adenylyl cyclase。

### Enzyme-Linked Receptors

受器本身即具酪氨酸激酶（tyrosine kinase）活性，或與之緊密相連。胰島素受器、leptin 受器（經 JAK-STAT 途徑）、生長因子受器等都屬此類。

**Table 75-2** 使用受器酪氨酸激酶訊號的激素：
...
### Adenylyl Cyclase–cAMP 系統

激素 → 受器 → Gs 蛋白 → Adenylyl cyclase 活化 → ATP → cAMP → Protein kinase A 活化 → 目標蛋白磷酸化 → 細胞效應

cAMP 一旦形成，會啟動酶的瀑布效應（cascade），使訊號逐級放大。

**Table 75-3** 使用 adenylyl cyclase–cAMP 系統的激素：
...
激素 → 受器 → Gq 蛋白 → Phospholipase C 活化 → PIP₂ → IP₃ + DAG

- **IP₃**：動員細胞內 Ca²⁺（粒線體、內質網）→ Calcium-calmodulin 途徑
- **DAG**：活化 Protein kinase C → 蛋白質磷酸化

DAG 的脂肪部分也是 arachidonic acid，是前列腺素（prostaglandin）與其他區域激素的前驅物。

**Table 75-4** 使用 phospholipase C 系統的激素：
...
細胞內 Ca²⁺ 濃度升高（10⁻⁸–10⁻⁷ → 10⁻⁶–10⁻⁵ mol/L）時，Ca²⁺ 與 calmodulin（四個鈣離子結合位點）結合並改變構形，進而活化或抑制 calmodulin 依賴性蛋白激酶。例如，活化 myosin light chain kinase 可引起平滑肌收縮。
